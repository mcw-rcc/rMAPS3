...
if __name__ == "__main__":
    # Legacy script entry point; prefer `python cli.py motif-map mxe ...` instead.
    pass
...
def getCombinedCounts(exon):
    ...
    countPerWindow(rVal[8], exon[region[6]], -1, eLen)
    countPerWindow(rVal[9], exon[region[7]], 1, iLen)
    countPerWindow(rVal[10], exon[region[8]], -1, iLen)
    countPerWindow(rVal[11], exon[region[9]], 1, eLen)
...
def plotMotifs_finale(mapName,
    ...
    canv.writePDFfile(mapsPath + '/' + 'ME.' + mapName + '.pdf')
    ## write to PDF

    canv.writeGSfile(mapsPath + '/jj.png', "png16m", resolution=80)
    os.rename(mapsPath + '/jj.png', mapsPath + '/' + 'ME.' + mapName + '.png')
...
def minPvalueOut(exonType):
    ...
    for fN in fileList:
        ...
        pValList.append([
            fileName[:-18], pValR01Chunk[0], pValR02Chunk[0], pValR03Chunk[0],
            pValR04Chunk[0], pValR05Chunk[0], pValR06Chunk[0], pValR07Chunk[0],
            pValR08Chunk[0], pValR09Chunk[0], pValR10Chunk[0], pValR11Chunk[0],
            pValR12Chunk[0]
        ])
