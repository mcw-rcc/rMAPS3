...
#  }
  my $flankingexon = $wds[0];
  my $longexon = $wds[1];

  my @wds2 = split(/\:/, $longexon);
  my $longExonEnd = $wds2[2];
  my $shortEE = $wds2[2];
  my $var   = $wds2[1];
 
  @wds2 = split(/\|/, $var);
  my $longExonStart_0base = $wds2[0];
  my $shortES   = $wds2[1];
...
while(defined(my $line = <INFILE>)) { 
   ...
   if ($strand eq "-"){
	  ...
	  $rMATS_longExonEnd=$rMATS_longExonEnd-1;
	  $rMATS_flankingES=$rMATS_flankingES-1;
      print OUTFILE "$rMATS_longExonEnd\t$rMATS_shortES\t";
      print OUTFILE "$rMATS_longExonEnd\t$rMATS_longExonStart_0base\t$rMATS_flankingES\t$rMATS_flankingEE\t";
   }
